...
{
private:
    /* 将构造函数定义为私有的，因此我们只能通过后面的creat静态函数来创建processpool实例 */
    processpool( int listenfd, int process_number = 8 );
public:
    /* 单体模式singleton，以保证程序最多创建一个processpool实例，这是程序正确处理信号的必要条件 */
    /* 单例的实现主要是通过：
    ...
    */
    ...
    ~processpool()
    {
        ...
        /* 侯杰讲过，array new（int new a[10]）要对应array delete（delete[]）
        因为delete会只进行一次dtor，只销毁第一个对象，
        delete[]会逐个调用dtor，销毁[]申请的空间。 */
    }
    /* 启动进程池 */ 
    void run();
    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    m_sub_process = new process[ process_number ];// 新建子进程类数组，保存子进程信息
...
void processpool< T >::run_child()
{
    ...
    while( ! m_stop )
    {
        ...
        for ( int i = 0; i < number; i++ )
        {
            ...
            {
                int client = 0;
                /* 从父子进程之间的管道读取数据，并将结果保存在变量clien里。
                如果读取成功，则表示有新客户连接到来 */
    ...
    //close( m_listenfd );
    /* 我们将这句话注释掉，以提醒读者：
    应该由m_listenfd的创建者来关闭这种文件描述符，即所谓的
...
void processpool< T >::run_parent()
{
    ...
    while( ! m_stop )
    {
        ...
        for ( int i = 0; i < number; i++ )
        {
            ...
            {
                /* 如果有新连接到来，就采用Round Robin方法将其分配给一个子进程处理 */
